...
phone_numbers = set(numbers_1) | set(numbers_2)
...
def get_std_dev_text(number, mean):
    calc = 0.0
    n = 0.0
    tweets = db_parsed.final_phonestrain.find({"phone_no":number})
    for tweet in tweets:
...
def get_num_truncated(number):
    count = db_parsed.final_phonestrain.find({"$and":[{"phone_no":number},{"truncated":True}]}).count()
    return count

def get_num_retweeted(number):
    count = db_parsed.final_phonestrain.find({"$and":[{"phone_no":number},{"retweeted":True}]}).count()
    return count

def get_num_verified(number):
    count = db_parsed.final_phonestrain.find({"$and":[{"phone_no":number},{"user.verified":True}]}).count()
    return count

def get_num_user_mentions(number):
    tweets = db_parsed.final_phonestrain.find({"$and":[{"phone_no":number},{"entities.user_mentions":{"$not": {"$size": 0}}}]})
    total_mentions = 0
    for tweet in tweets:
...
def get_num_in_replies(number):
    count = db_parsed.final_phonestrain.find({"$and":[{"phone_no":number},{"in_reply_to_screen_name":{"$ne": None}}]}).count()
    return count
...
def get_num_jumped_timezones(number, tweets):
    ...
    for account in accounts:
        timezones = db_parsed.final_phonestrain.find({"$and":[{"phone_no":number},{"user.screen_name":account}]}).distinct("user.timezone")
        if len(timezones)>1:
            count+=1
    return count

def get_total_hashtags(number):
    tweets = db_parsed.final_phonestrain.find({"$and":[{"phone_no":number},{"entities.hashtags":{"$not": {"$size": 0}}}]})
    total_hashtags = 0
    for tweet in tweets:
...
def insert_to_db(tweet_data):
    db_parsed.final_phone_features.insert(tweet_data)
